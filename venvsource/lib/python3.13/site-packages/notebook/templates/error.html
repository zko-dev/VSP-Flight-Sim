<!doctype html><html><head><meta charset="utf-8"><title>{% block title %}{{page_title | e}}{% endblock %}</title>{% block favicon %}<link rel="shortcut icon" type="image/x-icon" href="/static/favicons/favicon.ico">{% endblock %}<script defer="defer" src="{{page_config.fullStaticUrl}}/main.da4c5cf5c604f562794b.js?v=da4c5cf5c604f562794b"></script></head><body class="jp-ThemedContainer">{% block stylesheet %}<style>/* disable initial hide */
div#header, div#site {
    display: block;
